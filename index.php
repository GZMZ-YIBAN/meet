...
$query = $mysqli->query("select department,room,date,time,status from boardroom where DATEDIFF(date,now()) >= 0");
if($query == true){
...
if($_GET['act'] == 'login'){
   	header('location:https://oauth.yiban.cn/code/html?client_id=b19046a4baf4e76f&redirect_uri=http://f.yiban.cn/iapp169447&state=rains');
}
 $verify_request = '';
	...
	<div class="container">	
      ...
      <div class="panel panel-primary">
		...
		<div class="panel-body">
			<p class="alert alert-warning">请先对照下表并确定所要预约的时间，避免与其他组织预约的时间产生冲突，在审核时将按提交的先后顺序进行审核，时间冲突的将取消本次预约。<u>为了倡导实名登记，实名预约，现在需使用易班登录方可使用本系统！！</u></p>
			<div class="div"><label class="col-md-6"><a class="btn btn-warning btn_form" href='order.php'>立即申请预约</a></label>
			<label class="col-md-6"><a class="btn btn-info btn_form" href='query.php'>预约信息查询</a></label></div>
		</div>
	</div>
      <br />
		<div class="panel panel-info">
			...
			<table class="table table-striped table-hover table-bordered">
				...
				while($i < $total){
					echo "<tr>";
					echo "<td>{$list[$i]['department']}</td>";
					echo "<td>{$list[$i]['room']}</td>";
					echo "<td>{$list[$i]['date']} {$list[$i]['time']}</td>";
					if($list[$i]['status'] == -1){
